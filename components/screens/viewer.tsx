...
import { LabViewer } from "@/components/screens/lab-viewer"
...
export function Viewer({
  ...
  const hasLab = (video.labSteps?.length ?? 0) > 0
  const [tab, setTab] = React.useState<"video" | "lab">("video")
  ...
  React.useEffect(() => {
    ...
    // Default to the video tab when a video exists, otherwise show the lab.
    setTab(video.mp4Url || !hasLab ? "video" : "lab")
  ...
  return (
    <div className="flex h-full w-full flex-col bg-white">
      {/* Header: back link + mode tabs */}
      <div className="flex-none px-[18px] pt-[22px] md:px-[40px] md:pt-[28px]">
        <div
          onClick={onBack}
          className="flex w-fit cursor-pointer items-center gap-[10px] text-[13px] text-ink-3 transition-colors hover:text-ink"
        >
        ...
        {hasLab && (
          <div className="mt-4 flex gap-1.5 border-b border-line">
            {([
              { id: "video", icon: "film", label: "Video" },
              { id: "lab", icon: "book", label: "Interactive Lab" },
            ] as const).map((t) => {
              ...
              return (
                <button
                  ...
                  onClick={() => setTab(t.id)}
                  className={cn(
                    "flex items-center gap-2 border-b-2 px-3.5 py-2.5 text-[13.5px] font-semibold transition-colors -mb-px",
                    active
                      ? "border-ink text-ink"
                      : "border-transparent text-ink-3 hover:text-ink-2"
                  )}
                >
                  <Icon name={t.icon} size={16} className={active ? "text-ink" : "text-ink-3"} />
                  {t.label}
                </button>
              )
            })}
      ...
      {tab === "lab" && hasLab ? (
        <div className="min-h-0 flex-1">
          <LabViewer steps={video.labSteps!} title={video.labTitle || video.title} />
        </div>
      ) : (
        /* Video mode */
        <div className="flex-1 overflow-auto px-[18px] py-[22px] md:px-[40px] md:py-[24px]">
          {/* Video Player */}
